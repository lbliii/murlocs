...
2. Preserve existing intent.
   ...
   - Do not run `murlocs init` over an existing root `AGENTS.md`.
3. Draft `.murlocs/manifest.toml`.
...
4. Compile and verify.
   - Run `murlocs compile` only after reviewing the proposed manifest.
   - Run `murlocs check`; resolve manifest, graph, proof, coverage, budget, ownership, and drift findings.
   - Run `murlocs explain PATH` on representative files in each important scope.
...
Murlocs deliberately refuses unmanaged or modified generated maps. Do not bypass that protection by
...
the installed Murlocs version provides one.
