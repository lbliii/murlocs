...
def _task_command_result(payload: dict[str, Any]) -> CommandResult:
    """Shape one composite task envelope for compact, silent-capable terminals."""
    exit_code = 1 if payload["blocking"] else 0
    return CommandResult(
        payload,
        terminal_text="\n".join(render_task_lines(payload)),
        exit_code=exit_code,
        terminal_stream="stderr" if exit_code else "stdout",
    )
...
def orient_command(
    ...
    repo: Annotated[str, Option(metavar="PATH")] = ".",
    correlation_id: Annotated[str | None, Option(metavar="ID")] = None,
) -> OrientPayload | FailurePayload:
    """Orient before unfamiliar work on a repository path, read-only.

    Composes adoption, explanation, guidance health, and change routing for one
    path: applicable maps, invariants, guardrails, owners, provenance, focused
    checks, related one-hop scopes, context budget, blocking guidance-health
    findings, and one ordered next-action list. Registered checks are never
    executed and no repository state is mutated.

    Args:
        path: File or directory inside the repository to orient around.
        repo: Repository root containing the guidance network.
        correlation_id: Optional caller task/run id; never generated by Murlocs.
    """
    try:
        payload = build_orient(_root(repo), path, correlation_id=correlation_id)
    except MurlocsError as exc:
        return _failure("MURLOCS_ORIENT", exc)
    return _task_command_result(cast(dict[str, Any], payload))


def review_changes_command(
    path: Annotated[list[str] | None, Option(metavar="PATH")] = None,
    staged: bool = False,
    ...
    repo: Annotated[str, Option(metavar="PATH")] = ".",
    correlation_id: Annotated[str | None, Option(metavar="ID")] = None,
) -> ReviewChangesPayload | FailurePayload:
    """Review the guidance impact of an explicit change set, read-only.

    Routes exactly one explicit change view — repeated `--path` values or one of
    `--staged`, `--working-tree`, or `--revision-range` — to required and
    recommended scopes with reasons, owners, related boundaries, and focused
    checks. The exact repository and Git view appear in the structured and
    compact output. The command never guesses which changes belong to the task,
    and routing is a review signal, not a semantic-truth or merge-policy claim.

    Args:
        path: Explicit changed repository path. Repeat the option to review a set.
        staged: Review the staged index versus HEAD.
        working_tree: Review the working tree versus HEAD.
        revision_range: Review the changed paths of a Git revision range.
        repo: Repository root containing the guidance network.
        correlation_id: Optional caller task/run id; never generated by Murlocs.
    """
    try:
        payload = build_review_changes(
            _root(repo),
            paths=tuple(path or ()),
            staged=staged,
        ...
        return _failure("MURLOCS_REVIEW_CHANGES", exc)
    return _task_command_result(cast(dict[str, Any], payload))
...
def finish_command(
    ...
    working_tree: bool = False,
    revision_range: Annotated[str | None, Option(metavar="REVISION_RANGE")] = None,
    receipt_state_id: Annotated[str | None, Option(metavar="STATE_ID")] = None,
    repo: Annotated[str, Option(metavar="PATH")] = ".",
    correlation_id: Annotated[str | None, Option(metavar="ID")] = None,
) -> FinishPayload | FailurePayload:
    """Assemble a fresh structural completion receipt, read-only.

    Aggregates `check`, `impact`, and applicable curation validation over one
    explicit, freshness-bound Git view into one ordered list of blocking,
    authority-required, and recommended actions. Registered checks are named but
    never executed, a stale pre-edit receipt cannot satisfy completion, and the
    command never mutates, compiles, accepts, promotes, or repairs guidance.

    Args:
        path: Explicit completed repository path. Repeat the option for a set.
        staged: Complete against the staged index versus HEAD.
        working_tree: Complete against the working tree versus HEAD.
        revision_range: Complete against the changed paths of a revision range.
        receipt_state_id: Prior index-bound Git view state; a mismatch is stale.
        repo: Repository root containing the guidance network.
        correlation_id: Optional caller task/run id; never generated by Murlocs.
    """
    try:
        payload = build_finish(
            _root(repo),
            paths=tuple(path or ()),
            staged=staged,
            working_tree=working_tree,
        ...
        return _failure("MURLOCS_FINISH", exc)
    return _task_command_result(cast(dict[str, Any], payload))
...
def build_cli(*, name: str = "murlocs") -> CLI:
    ...
    app.command(
        "orient",
        description="Orient before unfamiliar work on a repository path",
        surfaces=("cli", "mcp", "llms"),
        annotations=inspection,
        terminal_renderer=_render_result,
    )(orient_command)
    app.command(
        "review-changes",
        description="Review the guidance impact of an explicit change set",
        surfaces=("cli", "mcp", "llms"),
        annotations=inspection,
        terminal_renderer=_render_result,
    ...
    app.command(
        "finish",
        description="Assemble a fresh structural completion receipt",
        surfaces=("cli", "mcp", "llms"),
        annotations=inspection,
        terminal_renderer=_render_result,
    )(finish_command)
